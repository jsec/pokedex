...
func (p *PokemonApi) AddToPokedex(name string) {
	fmt.Printf("Adding %v to pokedex...\n", name)
	details := p.GetDetails(name)
	p.pokedex[name] = details
}

func (p *PokemonApi) Inspect(name string) {
	details, ok := p.pokedex[name]
	if !ok {
	...
	fmt.Println("Stats:")
	for _, item := range details.Stats {
		fmt.Printf("  - %v: %v\n", item.Stat.Name, item.BaseStat)
	}
	fmt.Println("Types:")
	for _, kind := range details.Types {
		fmt.Printf("  - %v\n", kind.Type.Name)
	}
}

func (p *PokemonApi) Pokedex() {
...
func (p *PokemonApi) GetDetails(name string) PokemonResponse {
